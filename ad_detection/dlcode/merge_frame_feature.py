...
# -*- coding: utf-8 -*-
# hellolzc 20200201
""" Read features extracted by OpenSMILE, and save it in a HDF5 File.
The features are all LLDs.
...
def merge_LLDs(input_config, uuid, project_root):
    ...
    for indx, config in enumerate(input_config):
        ...
        if df_merged is None:
            ...
            cur_len = len(df_merged)
            if abs(len(df) - cur_len) > 3:
                print('\nWarnning: len0:%d, len1:%d' % (cur_len, len(df)))
            cur_len = min(cur_len, len(df))
            df_merged = pd.concat((df_merged.iloc[:cur_len], df.iloc[:cur_len]), axis=1)
...
def merge_LLDs_main(config_file_path):
    ...
    with h5py.File(output_file,"w") as h5file:
        for line in file_list:
            ...
            h5file.create_dataset(uuid, data=merged_LLD)
    print("\nLast output sample shape:", merged_LLD.shape)
